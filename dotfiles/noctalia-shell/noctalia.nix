...
let
  # Settings translated from ~/.config/noctalia/settings.toml (V5).
  # pkgs.formats.toml generates a validated settings.toml in the Nix store,
  # so a malformed value can't reach Noctalia's TOML validator at build time.
  noctaliaFormat = pkgs.formats.toml { };
  noctaliaSettings =
{
        audio = {
        ...
        shell = {
          clipboard_image_action_command = "/home/gonzo/Pictures/Clipboarded";
...
in
{
  ...
  home-manager.users.gonzo = {
    ...
    # Noctalia V5 settings, generated declaratively from the attrset above.
    programs.noctalia = {
      enable = true;
      settings = noctaliaFormat.generate "noctalia-settings.toml" noctaliaSettings;
    };
  };
}
